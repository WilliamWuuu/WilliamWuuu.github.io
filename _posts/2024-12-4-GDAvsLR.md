...
可见，生成式学习算法的核心目标是建模输入数据 $x$ 在给定标签 $y$ 下的条件分布 $p(x|y)$，以及标签的先验分布 $p(y)$。带入上面的情境，$p(x|y)=0$ 建模了猫的特征分布，而 $p(x|y)=1$ 建模了狗的特征分布。这一方法通过全面建模数据的生成机制，为预测问题提供了一种概率论基础。利用贝叶斯公式，生成式算法能够根据后验分布 $p(y|x)$ 进行预测: 

$$
p\left(y\middle|\ x\right)=\frac{p\left(x\middle|\ y\right)p\left(y\right)}{p\left(x\right)}
$$

其中，$p(x)$ 可通过 $\ p\left(x\right)=\sum\limits_{y}{\left(x\middle|\ y\right)p\left(y\right)}$ 进行计算。而实际上，由于

$$
\arg\max\limits _y p\left(y\middle|\ x\right)=\arg\max\limits _y p \left(x\middle|\ y\right)p\left(y\right)
$$

在进行预测即最大化 $p(y|x)$ 时，我们并不需要真正计算分母。
...
GDA假设 $p(x|y)$ 的条件分布是多元高斯分布，这一假设为模型提供了明确的数学框架。在给定 $y=0$ 或 $y=1$ 的条件下，输入特征 $x$ 分别服从两个不同的多元高斯分布，即
...
## 高斯判别分析与逻辑回归的区别
GDA模型和逻辑回归模型存在一个有趣的关系。如果我们将 $p(y=1|x;ϕ,μ0,μ1,Σ0)$ 视作 $x$ 的函数，可以发现它可以被表达为以下的形式：
...
可见，如果 $x|y$ 服从多元高斯分布(共享协方差矩阵 $Σ$)，那么 $p(y|x)$ 必然可以写成逻辑函数形式。然而反之则不成立，即使 $p(y|x)$ 是逻辑函数形式，也无法保证 $p(x|y)$ 是多元高斯分布。

这表明，GDA模型对数据的建模假设比逻辑回归更为强烈，具体体现在对条件分布 $p(x|y)$ 和共享协方差矩阵 $Σ$ 的高斯分布假设上。当这些假设与数据实际分布一致时，GDA 能够充分利用这些信息进行参数估计，因此在描述数据生成机制和预测准确性方面表现出显著优势。特别是，当 $p(x|y)$ 确实是多元高斯分布(共享 $Σ$)时，GDA 被证明是渐近有效的，即随着训练集规模的增大，没有任何其他算法能在估计后验概率 $p(y|x)$ 的准确性上超过GDA。在大样本情况下，这种优势尤其显著，使得GDA成为符合建模假设时的最佳选择。

相比之下，逻辑回归对数据分布假设的依赖较弱，因此在模型的灵活性和对噪声的鲁棒性上更具优势。逻辑回归不需要假设 $p(x|y)$ 的具体形式，只需直接建模 $p(y|x)$ 为逻辑函数。这使得逻辑回归能够适应各种数据分布。例如，即便 $x|y=0\sim P(λ_0)$，且 $x|y=1\sim P(λ_1)$ 分别服从非高斯的泊松分布，后验概率 $p(y|x)$ 依然可以用逻辑函数表示，从而保证逻辑回归模型的有效性和稳健性。
